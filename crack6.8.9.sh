...
bt_domain=https://moecat.cn/sh/bt6
...
MAIN_RETURNYS=${Red_font}[修复apche、nginx、ssl无法启动报错问题！]${Font_suffix}
...
starts0(){
	yum install -y wget && wget -O install.sh ${bt_domain}/install_6.0.sh && bash install.sh
...
echo -e "${Green_font}
#==========================================
 最近经常出现 apache nginx ssl启动 重装报错
 使用此命令 可以修复问题
 nginx 环境下 ssl 无法启动 用 nginx 修复
 apache 环境下 ssl 无法启动 用 apache 修复
 若是修复完了 重启还是报错 卸载环境重装就行
#==========================================
${Font_suffix}"
echo -e "${MAIN_RETURNYS}"
echo -e "1.nginx 修复\n2.apache 修复\n3.返回上目录"
read -p "输入数字以选择:" ys0
while [[ ! "${ys0}" =~ ^[0-3]$ ]]
	do
		echo -e "${Error} 无效输入"
		echo -e "${Info} 请重新选择" && read -p "输入数字以选择:" ys0
	done
if [[ "${ys0}" == "1" ]]; then
	ys1
elif [[ "${ys0}" == "2" ]]; then
...
echo -e "\033[43;35m nginx 修复操作已完成！ \033[0m \n" 
ys
}

ys2(){
mv /www/server/panel/vhost/apache/btwaf.conf  /tmp/btwaf.conf_back
mv /www/server/panel/vhost/apache/total.conf  /tmp/total.conf_back
echo -e "\033[43;35m apache 修复操作已完成！ \033[0m \n" 
ys
}
